...
func getMariaDBCurrentBinlogFile(db *sql.DB) (fileName string) {
...
func getMariaDBConnection() (*sql.DB, error) {
    datasourceName, err := internal.GetRequiredSetting(internal.MariadbDatasourceNameSetting)
    db, err := getMariaDBConnectionFromDatasource(datasourceName)
    if err != nil {
...
func getMariaDBConnectionFromDatasource(datasourceName string) (*sql.DB, error) {
    if caFile, ok := internal.GetSetting(internal.MariadbSslCaSetting); ok {
        ...
        if strings.Contains(datasourceName, "?tls=") || strings.Contains(datasourceName, "&tls=") {
            return nil, fmt.Errorf("MySQL datasource string contains tls option. It can't be used with %v option", internal.MariadbSslCaSetting)
        }
